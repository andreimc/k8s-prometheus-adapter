...
var nsGroupResource = schema.GroupResource{Resource: "namespace"}
...
func (n *metricNamer) QueryForSeries(series string, resource schema.GroupResource, namespace string, names ...string) (prom.Selector, error) {
	var exprs []string
	if namespace != "" {
		namespaceLbl, err := n.LabelForResource(schema.GroupResource{Resource: "namespace"})
		if err != nil {
			return "", err
		}
	...
	args := queryTemplateArgs{
		Series:        series,
		LabelMatchers: exprs,
		GroupBy:       []string{string(resourceLbl)},
	}
	...
	func() {
		...
		for lbl := range series.Labels {
			...
			if groupRes, ok = n.labelResExtractor.GroupResourceForLabel(lbl); ok {
				...
				resources = append(resources, info.GroupResource)
				updates[lbl] = info.GroupResource
			}
...
func (n *metricNamer) LabelForResource(resource schema.GroupResource) (pmodel.LabelName, error) {
	...
	n.labelResourceMu.RLock() // release before we call makeLabelForResource
	if ok {
		return lbl, nil
	}
...
func (n *metricNamer) makeLabelForResource(resource schema.GroupResource) (pmodel.LabelName, error) {
	...
	convResource.Group = groupNameSanitizer.Replace(convResource.Group)
...
func (n *metricNamer) MetricNameForSeries(series prom.Series) (string, error) {
	if n.naming.ConstantName != "" {
		return n.naming.ConstantName, nil
	}

	name := series.Name

	if !strings.HasPrefix(name, n.naming.Prefix) {
		return "", fmt.Errorf("series name %q does not have expected prefix %q", series.Name, n.naming.Prefix)
	}
	name = name[len(n.naming.Prefix):]

	if !strings.HasSuffix(name, n.naming.Suffix) {
		return "", fmt.Errorf("series name %q does not have expected suffix %q", series.Name, n.naming.Suffix)
	}

	name = name[:len(name)-len(n.naming.Suffix)]
...
func NamersFromConfig(cfg *config.MetricsDiscoveryConfig, mapper apimeta.RESTMapper) ([]MetricNamer, error) {
	...
	for i, rule := range cfg.Rules {
		labelTemplate, err := template.New("resource-label").Parse(rule.Resources.Template)
		if err != nil {
		...
		metricsQueryTemplate, err := template.New("metrics-query").Parse(rule.MetricsQuery)
		if err != nil {
			return nil, fmt.Errorf("unable to parse metrics query template %q associated with series query %q: %v", rule.MetricsQuery, rule.SeriesQuery)
		}
